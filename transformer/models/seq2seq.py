...
class Seq2SeqLM(BaseLM):
    ...
    def forward(
        ...
        # shape input/output: [batch, seq_len, model_dim]

        # lewatkan input, output, & mask-nya ke encoder-decoder
        ...
        # shape hidden: [batch, seq_len, model_dim]

        # proyeksikan balik ke output vocab size, pakai ulang bobot embedding (weight tying)
        unemb = self.model["output"][0].unembed(hidden)
        return nn.functional.log_softmax(unemb, dim=-1)
        # unembed/shape output: [batch, seq_len, output_vocab_size]
